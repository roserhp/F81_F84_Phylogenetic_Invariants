...
-----------------------------------------------------------------------------------------------------------
--
-- Quartet.m2: this file contains the following computations regarding quartets with tree topology 12|34:
--
...
"Auxiliary_files/qtilde1234.txt" << toString qtilde << endl << close
...
--non-zero coordinates of ptilde for 12|32 adding the four coordinates that can be non-zero in other tree topologies
nonZeroEntries=sort join(S_(positions(flatten entries ptilde,i->i!=0)),{(3,4,3,4),(4,3,4,3),(3,4,4,3),(4,3,3,4)})
...
flatt=matrix flatt;    
"Auxiliary_files/Flat1234_ptilde.txt" << toString flatt << endl << close
...
Tripod1=ideal(p_(1,1,4,4)*p_(1,2,2,2)-p_(1,1,2,2)*p_(1,2,4,4),
      p_(1,2,2,2)*p_(1,4,1,4)-p_(1,2,1,2)*p_(1,4,2,4),
      p_(1,2,2,2)*p_(1,4,4,1)-p_(1,2,2,1)*p_(1,4,4,2),
      p_(1,1,4,4)*p_(1,2,3,3)-p_(1,1,3,3)*p_(1,2,4,4),
      p_(1,3,2,3)*p_(1,4,1,4)-p_(1,3,1,3)*p_(1,4,2,4),
      p_(1,3,3,2)*p_(1,4,4,1)-p_(1,3,3,1)*p_(1,4,4,2),
      p_(1,1,3,3)*p_(1,3,1,3)*p_(1,3,3,1)-p_(1,1,1,1)*p_(1,3,3,3)^2,
      p_(1,1,4,4)*p_(1,4,1,4)*p_(1,4,4,1)-p_(1,1,1,1)*p_(1,4,4,4)^2,
      p_(1,2,3,3)*p_(1,3,2,3)*p_(1,3,3,2)-p_(1,2,2,2)*p_(1,3,3,3)^2)

Tripod4=ideal(p_(1,4,4,1)*p_(2,2,2,1)-p_(1,2,2,1)*p_(2,4,4,1),
      p_(2,2,2,1)*p_(4,1,4,1)-p_(2,1,2,1)*p_(4,2,4,1),
      p_(2,2,2,1)*p_(4,4,1,1)-p_(2,2,1,1)*p_(4,4,2,1),
      p_(1,4,4,1)*p_(2,3,3,1)-p_(1,3,3,1)*p_(2,4,4,1),
      p_(3,2,3,1)*p_(4,1,4,1)-p_(3,1,3,1)*p_(4,2,4,1),
      p_(3,3,2,1)*p_(4,4,1,1)-p_(3,3,1,1)*p_(4,4,2,1),
      p_(1,3,3,1)*p_(3,1,3,1)*p_(3,3,1,1)-p_(1,1,1,1)*p_(3,3,3,1)^2,
      p_(1,4,4,1)*p_(4,1,4,1)*p_(4,4,1,1)-p_(1,1,1,1)*p_(4,4,4,1)^2,
      p_(2,3,3,1)*p_(3,2,3,1)*p_(3,3,2,1)-p_(2,2,2,1)*p_(3,3,3,1)^2)

betti Tripod1
betti Tripod4

--All ranks and tripod equations
CI=trim(CI0+CI1+CI2+CI3+Tripod1+Tripod4);
betti CI --4 linear, 40 quadrics, 10 cubics, 14 quartics

"Auxiliary_files/QuartetCI_ptilde.txt" << toString CI << endl << close
